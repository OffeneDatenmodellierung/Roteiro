...
two things that do not exist, and a back control would be a control that goes
nowhere — not because `/` 404s, since `serve_okf_only` redirects it to the mount
base, but because following it lands the reader back in this same viewer, which
is worse than a dead link because it looks like it worked. That is exactly the
slide into "a viewer for *our* bundles" this ADR exists to refuse. Standalone keeps the v1.0 header, with the bundle directory's own name
...
| 1.5 | 2026-09-14 | Amended (no issue; maintainer decision). **The mounted viewer's chrome caught up with the fold v1.2 performed.** The viewer has been served by the same binary, in the same process, on the same port as the explorer since v1.2, and nothing in its header said so: it printed `self.root.display()` — an absolute path under the operator's home directory — where the shell's project bar prints `Roteiro · Workspace ▸ <project>`, and it offered no control that walked back out to the workspace. A reader who arrived on it could reasonably conclude the viewer was a separate thing that happened to share a port. It now wears the shell's own bar: `assets/index.html`'s `p-back`, `p-crumbs`, `p-crumb-root`, `p-crumb-link`, `p-sep` and `p-crumb-current`, with the same declarations and the v1.3 tokens, and a trail of `Roteiro · Workspace ▸ OKF ▸ <label>` in which `OKF` is a link to the chooser when this server holds more than one bundle and a plain step when it does not — the condition `Nav::bundles` already encodes, because a one-row chooser redirects straight back. The breadcrumb *is* the way up, so the mounted form stops emitting the separate `Explorer` and `All bundles` links it used to: two controls for one destination is the second pattern this deliberately does not invent. The name in the trail is `Mount::label` verbatim, the `<workspace>/<project>` string `okf_mounts` had already resolved for the chooser and the startup line; nothing re-derives it. **The premise v1.2 nearly lost is the thing this change had to be careful with.** `serve_okf_only` passes `explorer: None` because it is the case where a bundle is a stranger's, at an arbitrary path, with no repository and no workspace anywhere above it. A breadcrumb reading `Roteiro · Workspace ▸ …` there names two things that do not exist, and a `← Workspace` control would go nowhere — **not** because `/` 404s, which was this entry's first and wrong explanation: `serve_okf_only` defines `/` as a redirect to the mount base, so following that control would land the reader back in the same viewer, which is worse than a dead link because it looks like it worked. So the standalone form gets neither, keeps v1.0's `name`/`root` header, and keeps its `All bundles` link, since a standalone server can still hold several bundles and its chooser is real. Both forms are pinned verbatim rather than by substring, because the risk here is precisely a later change that improves one and quietly gives the other a trail to nothing. **And the path stopped being published.** `serve` takes `--addr` and #832 added `--scope`, so a non-loopback bind is expressible in both forms. The header was **one of two** places a mounted bundle page carried the absolute path, and the first draft of this entry claimed it was the only one — twice, each time narrowed and each time still false, because the second place is reached only when the bundle **breaks**. All four content handlers answer a bundle that has stopped loading through `unreadable`, which rendered `InspectError::Unreadable` into the response body; that error names the path because it is a *CLI* error and `roteiro okf inspect <path>` must say which path failed, and `okf-core`'s `detail` beside it named it a second time. A bundle is somebody else's directory and may be moved or unmounted while a server holds it, so this is the ordinary failure rather than an exotic one. It is now **split by audience** rather than softened: the operator gets the whole error on stderr, where the path is the useful part and the reader can act on it, and the client gets the fact plus the bundle's own label, which the header already shows them. **Once per outage, not once per response** — the first version reported from `unreadable`, which every content route reaches, so a bundle that had gone away wrote one line per request at whatever rate the caller chose; on a non-loopback bind that is an unauthenticated stranger deciding how much synchronous, globally-locked stderr I/O this process does and how fast whatever collects it fills. The report moved to `with_cache`, the one place that knows an *attempt* just failed, behind a latch cleared on every successful load so a later outage is reported rather than swallowed by the first. The page says the bundle **could not be read** rather than that it is *no longer* readable, because mount admission checks only that an `index.md` exists — so the first load can fail and this is then the page for a bundle that was never readable at all, to whom "no longer" says something untrue about what happened. The split is made at the presentation boundary and not in `InspectError`, which is right for its own callers — what was wrong was one HTTP response treating a diagnostic written for an operator as a page written for a stranger. `no_mounted_page_publishes_the_bundle_path` walks every route a mounted bundle answers on — index, graph page, `api/graph.json`, a concept, a 404 — and asserts the bundle's real root appears in none of them, and `no_mounted_error_page_publishes_the_bundle_path_either` walks the same routes against a bundle that has been removed underneath the server. That second test had to be built carefully: its first draft made the bundle "unreadable" by deleting `index.md`, got four `200 OK`s because `okf-core` loads a bundle without one, and would have passed while measuring nothing — so it now asserts each response really is a 404 that says it is unreadable before scanning it, and `the_operator_learns_which_bundle_broke_once_per_outage` drives a real outage, a recovery and a second outage through the cache, **counting the diagnostics** rather than inferring them from the latch. That distinction was itself a review finding: the first version asserted on the latch, and a regression keeping the `swap` while moving the write out from behind it flips the latch exactly as a correct implementation does — green test, one line per request restored. The latch is the mechanism; the property is the number of writes, so the write now goes through a sink a test can count. A counter on one sink cannot see a *second* write appearing beside it, so `the_viewer_has_one_diagnostic_path` holds the other half, and the two are complementary rather than redundant — each injection reddens exactly one of them. **And that line is remote-triggered text the operator reads**, so it is escaped before it is written: `InspectError`'s `Display` interpolates a bundle path and `okf-core`'s parser detail, which quotes the bundle's own bytes, and a bundle is somebody else's — a newline in there forges whole log lines and an `ESC` writes colour, moves the cursor or clears the screen of whatever terminal is tailing the log. Escaped rather than stripped, so the operator still sees that something strange was in the name instead of reading a silently-edited one. **Both lateral navs also gained accessible names.** The mounted breadcrumb was already a labelled `Breadcrumb` landmark; adding a second and sometimes a third unlabelled `<nav>` beside it left a screen-reader user with a list of landmarks they could not tell apart, which is a regression this change introduced rather than one it inherited. Standalone now shows the bundle directory's own name instead: the leading segments described the host rather than the bundle, and were of no use to a reader who was sent the URL. **The mount base above those pages still prints one, and is deliberately untouched** — so "stopped being published" is a claim about bundle pages and not about this server's whole surface. The chooser lists `Mount::origin` beside each bundle, a v1.2 column whose stated job is telling two similarly-named bundles apart after `slug` folds them; it renders only when more than one bundle is mounted, which is why a single-bundle server shows no path anywhere. Whether that is worth the same disclosure is a decision about the chooser rather than about the header, and it is tracked as a live one in #836, so it is stated here rather than assumed — `the_chooser_still_names_each_bundles_directory` pins that it does, and the fixture that feeds it was corrected to carry a real path, because one saying `"test"` made the scan pass by having nothing to find. |
