...
- **Execution:** models loaded lazily on first request and kept warm in a memory-bounded LRU — the resident set is capped by a byte budget (`[serve] memory_budget_mb`, GGUF size as the footprint proxy) and the least-recently-used model is unloaded past it, so several models can stay warm and swap in real time on a machine that can afford them, while a memory-limited host keeps one. Requests are serialised per model initially (llama.cpp supports Metal batching — a later enhancement). Serves only installed models; never downloads.
