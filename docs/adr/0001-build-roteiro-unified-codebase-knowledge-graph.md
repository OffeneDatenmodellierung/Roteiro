...
- **[[docs/adr/0026-research-knowledge-layer.md]]** v0.2 — a research note a
  ...
  difference is the point: a `knowledge/` page **would be** a graph fact, with a
  source blob and — once ADR-0026's step 2 lands — a drift check, so the "not a
  graph fact" test does not dispose of it. (ADR-0026 is `For Review` and unbuilt;
  [[crates/rto-spec/src/layer.rs]] has no `knowledge` arm today. The decision
  recorded here is about the class such a page takes, not about a page that
  exists.)
...
a fourth variant is anticipated, when four consecutive ADRs establish that it is
