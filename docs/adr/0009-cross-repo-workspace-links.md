...
| 1.18 | 2026-08-26 | **The shareable manifest carries version pins** (issue #442, completing step 8's reach into the vault). The manifest reconstructs the workspace *as rendered* — clone here, check out these commits — but that answers a different question from *what is each deployment repo running*, which is rarely the commit it was rendered at. A spoke at `HEAD` deploying `app@1.4.0` describes a live system its own commit id says nothing about. `_Home`'s `### Version pins` table now records, per member, the hub revision it deploys and where that was read from (`MemberPin`, rendered by `write_pins`, gathered by `workspace_pins` — all three removed with the vault in v1.19; the names are kept unlinked because this row records what was decided on 2026-08-26, and rewriting it would make the history a description of the present). **Each pin names its hub**, and is resolved against that member's own **parent** in [[crates/rto-graph/src/topology.rs#project_graph]] rather than against the workspace's busiest node — the v1.15/v1.17 snowflake makes "the hub version this member pins" ambiguous otherwise, and taking the busiest node as everyone's hub would report an infra repo as pinning a version of an application it has never referenced. One row per **dependency**, not per member: a project with two parents pins each separately. **A dependency whose version cannot be resolved is reported as `(none detected)` rather than dropped**, and the caption counts found against asked (`1 of 2 dependencies resolve`) — the rule #505 established, and the one that matters most here, because before #609 the image half of detection could not fire at all for a Helm-shaped spoke, so a workspace of entirely unresolved pins is exactly the state that used to be invisible. A pin follows a **declared** dependency: the table reads persisted `external_ref` edges and does not infer, so a workspace that has been synced but never `links --write`-ten shows no section at all — consistent with the section being part of a manifest, where an inferred name match is not a claim worth reproducing. Verified end to end against a spoke with **no Dockerfile**, declaring `image.repository`/`image.tag` in `values.yaml` as a chart does. |
| 1.19 | 2026-08-28 | **Version pins move from the vault's `_Home` to an OKF bundle** (issue #663). The Obsidian renderer is removed in 4.0.0, and with it `MemberPin`/`write_pins`/`workspace_pins` — so the v1.18 mechanism no longer exists in the form that row describes. What it decided is unchanged and still holds: a pin names its hub, resolves against that member's own parent in [[crates/rto-graph/src/topology.rs#project_graph]], is one row per **dependency** rather than per member, and an unresolved dependency is reported rather than dropped. What changed is only where it is written. A workspace bundle nests by member (`/<member>/<section>/`), which is what the vault's `<project>::<key>` qualification and per-note hashing existed to achieve on one flat directory — so the naming scheme is retired rather than ported. **Not yet re-implemented:** the pins table itself has no OKF home yet; the bundle carries per-member concepts but not the cross-member manifest `_Home` held. That is a gap this row records rather than hides, because a manifest nobody rebuilt is exactly the kind of loss a version history is for. |
| 1.20 | 2026-09-01 | **A placeholder can now hold the concept it stands in for** (issue #706, phase 1). v1.19 recorded that a workspace bundle carries per-member concepts; this records the reverse direction. An `extref:<project>::<key>` node was defined here as a stand-in for a node the referring member *cannot see* — a key, a `meta.qualified` target, and nothing else — so an authored `[[links]]` reference resolved to a document whose entire content is that it is not the document the reader wanted. When the target member publishes an OKF bundle, `roteiro import --from okf` fills the stub in place: the node keeps its `extref:` key and its `meta.qualified`, so [[crates/rto-graph/src/workspace.rs#Workspace]]'s `follow_external_ref` still resolves across repos exactly as before, and gains the peer's kind, name and prose. **Isolation is unchanged and is the reason the fill takes this shape.** No cross-store edge is created and no graph is merged: the peer's content is copied into *this* store as an import layer under its own `src_ref`, tagged `external-*` so it can never be mistaken for this repo's own work, and removable by re-importing a smaller bundle. The correspondence between a placeholder key and a bundle path is computed **forwards** through the writer's `slug`/`section_for` rule rather than by inverting a filename — `slug` is lossy, two keys can produce one name, and an ambiguous match fills nothing, because attaching a peer's content to the wrong node is worse than leaving the stub empty. The placeholder's own provenance remains, as v1.x already recorded, whichever layer wrote it last and not a statement about any link; the **edges** still carry that. |
