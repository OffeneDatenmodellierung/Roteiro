...
<!--
Stable anchors (issue #457). A cross-reference below points at `#stage-N`, an
explicit `<a id>` planted above the heading — not at the heading's slug.

Why: the slug encodes the whole heading, status parenthetical included, so
`… → **v0.11.x** ✅ *next*` becoming `… ✅ *delivered*` silently killed every
link into it. Six links died that way, in this document, on GitHub as well as on
roteiro.dev. An explicit id is not a function of the prose, so editing a stage's
status can no longer break a link to it.

Why an `<a id>` rather than the `{#anchor}` heading attribute this repository
uses on `website/pages/*.md`: that attribute is the *renderer's* extension, and
GitHub prints it verbatim in the heading and folds it into the slug. An `id`
works in both — GitHub rewrites it to `user-content-stage-N` and resolves bare
`#stage-N` links against that, exactly as it does its own heading anchors
(measured against github.com's HTML, not assumed).

Can the class recur? Yes — any *new* `](#…)` written against a heading slug is
one status edit from dying, and nothing checks it: `roteiro check` validates ADR
wiki-links, `@rto:` annotations and ADR version drift, and has no opinion about
an ordinary intra-document anchor. There is no guard. Issue #459 is where one
belongs.
-->
...
hardening that once lived here is now tracked explicitly as [Stage 14](#stage-14).)*
...
- **Delivered:** ADR-0003 amended to v1.1 recording the candle→llama.cpp
