...
//!
//! [`crate::Producer`] (ADR-0015) is not a label but a *verifiable identity*: it
//! folds a `model_digest` **as pinned in the registry** into a canonical id,
//! which is what makes re-describing an image with different weights a new
//! record rather than a silent overwrite.
//!
//! **That does not transfer to a hosted model, and pretending it does would be
//! the dishonest part of that feature.** A hosted model has no digest anyone can
...
//! is displayed.
//!
//! # Why it lives here rather than in `rto-remote`
//!
//! It was written in `rto-remote` alongside the egress ledger that first
//! recorded it, and it moved here when [`crate::ModelSource::Remote`] needed to
//! carry it. The move was forced rather than chosen: `rto-remote` depends on
//! this crate, so a trust grade named by a `rto-graph` type cannot live there,
//! and the alternative — a second, parallel enum — would let a record's trust
//! and a resolution's trust disagree about what "vendor-asserted" means.
//!
...
//! grade is a property of *producer identity*, not of the transport that
//! happened to introduce the ungradeable case. `rto-remote` re-exports it, so
//! `rto_remote::ProducerTrust` still resolves and there is exactly one type
//! behind both paths.
//!
//! Nothing here reaches the network, and this crate still cannot: a trust grade
//! is a two-variant enum and a sentence.
