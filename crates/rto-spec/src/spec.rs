...
pub fn scaffold_adr(
    ...
    // **The scaffold and the formatter are two writers of one document**, so
    // they have to agree: a fresh scaffold that `roteiro docs fmt` reports as
    // drifted is one nobody can commit without the canonical-doc gate failing.
    //
    // Routed through the canonical form rather than hand-matched, because
    // matching by hand is what produced the disagreement — the same lesson as
    // `rto_serve::openai_params::published_table`, which generates into
    // `docs/SERVING.md` and had this defect first (ADR-0023 v1.0).
    crate::fmt::canonical(&out)
}
...
mod tests {
    ...
    /// A fresh scaffold is already in canonical form.
    ///
    /// The scaffold and `roteiro docs fmt` are two writers of one document. A
    /// scaffold that the formatter reports as drifted is one nobody can commit
    /// without `docs_are_canonical` failing — so the generator is routed
    /// through [`crate::fmt::canonical`] rather than hand-matching it, and this
    /// is what holds the two together. Raised on #790, after the same defect in
    /// `rto_serve::openai_params::published_table`.
    #[test]
    fn a_fresh_scaffold_is_canonical() {
        use super::{scaffold_adr, scaffold_blueprint};
        let store = seeded();
        let ctx = context(&store, "validate_token", 10).expect("context");
        for md in [
            scaffold_adr("validate_token", Some("T"), "0099", "2026-08-09", &ctx),
            scaffold_blueprint("validate_token", Some("T"), &ctx),
        ] {
            assert_eq!(
                crate::fmt::canonical(&md),
                md,
                "a fresh scaffold is not canonical — `roteiro docs fmt` would \
                 report drift on a file the tool just wrote"
            );
        }
    }
