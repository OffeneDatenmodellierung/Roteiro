...
impl GraphServer {
    ...
    async fn check(&self, Parameters(args): Parameters<CheckArgs>) -> CallToolResult {
        ...
        // The path policy is empty by necessity, not by oversight, and for the
        // same reason `debt` below passes no ignore list: this crate cannot read
        // the target project's `roteiro.toml`. The binary's own MCP surface,
        // which can, passes the real one. A repository that declares `[paths]`
        // should reach its graph through that surface.
        query_result(self.with_project(project, |store| {
            rto_spec::tool_check(store, root.as_deref(), rto_graph::PathPolicy::empty())
        }))
    }
