...
pub struct Finding {
    ...
    /// The file the finding is about, if it names one.
    pub path: Option<String>,
    /// What is wrong.
    pub message: String,
    /// Whether upstream's `okf fix` could repair this automatically.
    ///
    /// Reported, not acted on. Repairing a bundle is a write, and this whole
    /// module reads.
    pub fixable: bool,
}
...
impl CheckReport {
    ...
    /// would be unusable against real third-party bundles.
    #[must_use]
...
/// Check a bundle for conformance with the specification.
///
/// Deterministic: `stale_after` is checked for syntax but never against the
/// clock, so the same bundle validates the same way tomorrow. A check whose
/// result depends on when it ran cannot be a CI gate.
///
/// # Errors
///
/// [`InspectError::Unreadable`] if the path is not a loadable OKF bundle.
...
pub fn validate_report(root: &Path) -> Result<CheckReport, InspectError> {
    ...
    Ok(findings(
        root,
        "validate",
        &okf_validator::validate_bundle(&bundle),
    ))
}

/// Check a bundle against the hygiene rules (`L1`..`L12`).
///
/// Distinct from [`validate_report`]: conformance asks whether the bundle *is*
/// OKF, linting asks whether it is *good* OKF. They share a feature gate only
