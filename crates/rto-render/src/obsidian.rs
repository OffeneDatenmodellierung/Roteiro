...
/// `tags` for the graph view and an ADR's `status`), the captured content as the
/// knowledge base, and its edges as provenance-labelled wikilinks.
#[must_use]
...
pub fn render_home(s: &VaultSummary) -> VaultNote {
    ...
    c.push_str("provenance-tagged: `derived` (extracted), `authored` (intent), ");
    c.push_str("or `inferred` (a scored suggestion).*\n");
    let _ = writeln!(
        c,
