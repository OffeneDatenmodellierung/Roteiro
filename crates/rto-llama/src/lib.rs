...
//! without the C++ engine, which lands behind the `llama` feature. What that
//! feature brings is named here rather than linked — `llama::LlamaEngine` above
//! and `backend` below — because this page is the one a reader of a build
//! *without* `llama` sees, and a link would be unresolved in exactly the
//! configuration this paragraph describes.
//!
